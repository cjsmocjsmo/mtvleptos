...
#[component]
pub fn TVAlteredCarbonSeaPage() -> impl IntoView {
    ...
    view! {
        <div class="seaMainDiv">
            <h1 class="seaH1">Altered Carbon</h1>
...
async fn fetch_episodes_s1() -> Result<Vec<Episode>, Error> {
    let response = reqwest::get("http://10.0.4.41:7777/alteredcarbon1").await?;
    let episodes: Vec<Episode> = response.json().await?;
    Ok(episodes)
}
async fn fetch_episodes_s2() -> Result<Vec<Episode>, Error> {
    let response = reqwest::get("http://10.0.4.41:7777/alteredcarbon2").await?;
